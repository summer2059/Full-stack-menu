...
@push('css')
    <style>
        ...
        .stat-card.total::after  { background: #2563eb; }
        .stat-card.low    { background: #fef2f2; }
        .stat-card.low::after    { background: #dc2626; }
        .stat-card.value  { background: #f0fdf4; }
        .stat-card.value::after  { background: #16a34a; }
        .stat-card.today  { background: #fffbeb; }
        .stat-card.today::after  { background: #d97706; }
    </style>
@endpush
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('inventory.restock') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Quantity to Add <span id="restock_unit_label" class="text-muted"></span></label>
                        <input type="number" name="quantity" class="form-control" step="0.01" min="0.01" required placeholder="e.g. 10">
                    </div>
                    <div class="mb-3">
        ...
        columns: [
            ...
            { data: 'unit', name: 'unit' },
            {
                ...
                render: function(data, type, row) {
                    return `<span class="fw-semibold">${parseFloat(data).toFixed(2)} ${row.unit}</span>`;
                }
            },
            {
                data: 'minimum_stock', name: 'minimum_stock',
                render: function(data, type, row) {
                    return `${parseFloat(data).toFixed(2)} ${row.unit}`;
                }
            },
            {
